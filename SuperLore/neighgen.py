...
class CounterGenerator(NeighborhoodGenerator):
    ...
    def find_closest_counter(self, counter_list, x):
        ...
        for feat in counter_list.keys():
            ...
            for new_record in counter_list[feat]:
                ...
                try:
                    ...
                    distance = abs(new_record[0][feat] - x[0][feat])
                if distance < closest:
                    closest = distance
        ...
        new_c = list()
        print('CLOSEST ', len(closests), closests)
        for c in closests:
            ...
            while cicla:
                try:
                    ...
                    if c[1][0][c[0]] < x[0][c[0]]:
                        trial[0][c[0]] = np.random.uniform(low=c[1][0][c[0]], high=x[0][c[0]], size=1)
                    else:
                        trial[0][c[0]] = np.random.uniform(low=x[0][c[0]], high=c[1][0][c[0]], size=1)
    ...
    def create_counter_on_generate(self, x, num_samples=1000):
        counter_list = dict()
        possibilities = dict()
        counter_found = False
        try:
        ...
        for f in feature_bins.keys():
            ...
            for val in actual['avg']:
                ...
                if pred_actual != pred_x:
                    print('in generate, sono nel caso in cui pred actual e diversa da original', temp, counter_list)
                    ...
                    if pred_proba_actual[0][pred_x] < pred_proba_x[0][pred_x]:
                        if f in possibilities.keys():
                            ...
                            possibilities[f].append(temp)
        if counter_found:
            print('the counter found is true', counter_list, x)
        ...
        '''geneticp = GeneticProbaGenerator(self.bb_predict, self.feature_values, self.features_map,
                                        self.nbr_features, self.nbr_real_features, self.numeric_columns_index,
                                        ocr=self.ocr, alpha1=self.alpha1, alpha2=self.alpha2, ngen=self.ngen,
                                        mutpb=0.5, cxpb=0.7, tournsize=self.tournsize, halloffame_ratio=self.halloffame_ratio,
                                        bb_predict_proba=self.bb_predict_proba,
                                        random_seed=self.random_seed, encdec=self.encdec, verbose=True)
        ...
        Z = geneticp.generate(x, num_samples=1000)
        ...
        for ind in range(0, self.max_counter):
            ...
            Z_temp = (geneticp.generate(rec, num_samples=500))
...
class GeneticGenerator(NeighborhoodGenerator):
    ...
    def fit(self, toolbox, population_size):
        ...
        print('qui i valori di cxpb e mutpb', self.cxpb, self.mutpb, halloffame)
...
class CFSGenerator(NeighborhoodGenerator):
    def __init__(self, bb_predict, feature_values, features_map, nbr_features, nbr_real_features,
                 numeric_columns_index, ocr=0.1, n_search=10000,n_batch=1000,lower_threshold=0,upper_threshold=800,
    ...
    def binary_sampling_search(self, x, x_label, downward_only=True, **kwargs):
        ...
        #print('prima del for ', self.lower_threshold, self.upper_threshold)
        while self.lower_threshold / self.upper_threshold < self.stopping_ratio:
            ...
            for i in range(int(self.n / self.n_batch)):
                ...
                if not np.all(y == x_label):  # if we found already some counterfactuals
                    #print('siamo nell if corretto probelma dobbiamo uscire ma no ', threshold)
                ...
                #print('Nel ciclo for ', self.lower_threshold, self.upper_threshold)
        if self.verbose:
        ...
        if self.forced_balance_ratio is not None:
            ...
            if (n_minority_instances / self.n) < self.forced_balance_ratio:
                ...
                while n_minority_instances < n_desired_minority_instances:
                    ...
                    y_ = 1 * (y_ == x_label)
                    #print('la y meno che ci sta problemando ', np.unique(y_, return_counts=True))
                    try:
                        n_minority_instances += np.unique(y_, return_counts=True)[1][minority_class]
                    except:
                        n_minority_instances = 0
